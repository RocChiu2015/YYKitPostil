...
- (void)setImageURL:(NSURL *)url {
    ...
    //YYAnimatedImageView类既能加载动态图片也能加载静态图片
    
    //加载网络静态图片
//    _webImageView.imageURL = [NSURL URLWithString:@"https://www.baidu.com/img/bd_logo1.png"];
    //加载网络动态图片
//    _webImageView.imageURL = [NSURL URLWithString:@"http://i.imgur.com/uoBwCLj.gif"];
    //加载本地静态图片
//    NSString *path = [[NSBundle mainBundle].bundlePath stringByAppendingPathComponent:@"EmoticonWeibo.bundle/com.sina.default/d_guzhang@3x.png"];
//    _webImageView.imageURL = [NSURL fileURLWithPath:path];
    //加载本地动态图片
//    NSString *path = [[NSBundle mainBundle].bundlePath stringByAppendingPathComponent:@"niconiconi@2x.gif"];
//    _webImageView.imageURL = [NSURL fileURLWithPath:path];
    
    // 1. 下载图片
    // 2. progress:获得图片下载进度
    // 3. transform:调整图片大小、加圆角
    // 4. 显示图片时增加一个淡入动画，以获得更好的用户体验
    // 5. completion:图片下载完成以后要做的事情
    [_webImageView setImageWithURL:url
                       placeholder:nil
                        ...
                        completion:^(UIImage *image, NSURL *url, YYWebImageFromType from, YYWebImageStage stage, NSError *error) {
                            NSLog(@"image = %@", image);
                            NSLog(@"url = %@", url);
                            NSLog(@"from = %lu", (unsigned long)from);
